...
public class GetCausalNodesTest {
    ...
    public void calculatesCausalNodesBetweenTwoEvents() throws Throwable {
        try (Driver driver = GraphDatabase.driver(neo4j.boltURI(), Config.build()
            ...
            testResult(this.neo4j.getGraphDatabaseService(),
                            "MATCH (start {eventId: 'cloud83.cluster.lsd.di.uminho.pt1'}), (stop {eventId: 'cloud83.cluster.lsd.di.uminho.pt10'}) " +
                            "CALL horus.getCausalNodes(start, stop) YIELD node " +
                            "RETURN node",
                    res -> {
                        String nodeId;

                        String[] causalNodeIds = {
                                "cloud83.cluster.lsd.di.uminho.pt1",
                                "cloud83.cluster.lsd.di.uminho.pt2",
                        ...
                        List<String> actualNodeIds = res.stream().map(node -> (String) ((NodeProxy) node.get("node")).getProperty("eventId")).collect(Collectors.toList());
